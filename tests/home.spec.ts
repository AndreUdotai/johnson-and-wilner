...
test.describe('Home Page', () => {
  ...
  test('should display the main navigation links', async ({ page }) => {
    ...
    await expect(nav.getByRole('link', { name: /practice areas/i })).toBeVisible();
    await expect(nav.getByRole('link', { name: /publications/i })).toBeVisible();
  ...
  test('should show the firm name in the hero section', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'JOHNSON & WILNER LLP - BROKEN', exact: true })
    ).toBeVisible();
  });
});